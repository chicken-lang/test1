...
/**
 * 侧边栏热门/推荐项(轻量)
 * V2.0字段标准: id→articleId, publishDate→publishedAt, views→viewCount
 */
export interface SideItem {
  articleId: number  // V2.0: id → articleId
  title: string
  publishedAt: string  // V2.0: publishDate → publishedAt
  viewCount: number  // V2.0: views → viewCount
  url: string
}
