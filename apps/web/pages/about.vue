...
// 部门概况页(T4.4) - 对齐需求文档一级栏目"部门概况"
// 包含: 部门简介 + 部门领导 + 7大业务口结构 + 人员一览 + 主要职责 + 联系我们
// 数据源: useApi → /api/about（SSR 预取，返回 {brief, history, leaders, divisions, duties, contact}）
const api = useApi()
const { data } = await useAsyncData('about', () => api.get<any>('/about'))
const deptIntro = computed(() => ({
  brief: data.value?.brief ?? '',
  history: data.value?.history ?? '',
}))
const deptLeaders = computed(() => data.value?.leaders ?? [])
const businessDivisions = computed(() => data.value?.divisions ?? [])
const mainDuties = computed(() => data.value?.duties ?? [])
const deptContact = computed(() => data.value?.contact ?? null)
...
const divisionOptions = computed(() => [
  ...
  ...businessDivisions.value.map((d: any) => ({ value: d.name, label: d.name })),
])

const filteredStaff = computed(() => {
  if (filterDivision.value === 'all') return businessDivisions.value
  return businessDivisions.value.filter((d: any) => d.name === filterDivision.value)
})

useSeoMeta({
  title: '部门概况 - 深圳信息职业技术大学教务处',
  description: '深圳信息职业技术大学教务处部门概况,含部门简介、领导介绍、机构设置、主要职责及联系方式',
})
</script>
...
.contact-card {
  ...
  &::before {
    ...
    height: 200px;
    border-radius: 50%;
    background: rgba(0, 115, 189, 0.08); // VI 主色半透明装饰圆(VI 合规:纯色,无渐变)
  }
}
