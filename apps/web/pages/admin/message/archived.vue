      ...
      <el-table-column label="操作" width="180" align="center" fixed="right">
        <template #default="{ row }">
          <div class="table-actions">
            ...
            <el-button type="warning" link size="small" @click="handleUnarchive(row)">
              取消归档
            </el-button>
          </div>
        </template>
...
async function handleUnarchive(row: any) {
  try {
    await api.put(`/messages/${row.id}/unarchive`)
    ElMessage.success(`已取消归档：${row.title}`)
    overrides.value[row.id] = { ...overrides.value[row.id], isArchived: false }
    tableData.value = tableData.value.filter((m: any) => m.id !== row.id)
    pagination.total = Math.max(0, pagination.total - 1)
  } catch (err: any) {
    ElMessage.error(err?.message || '取消归档失败')
  }
}
