...
// 数据源: useApi → /api/feedback/types + /api/feedback（SSR 预取）
import { ElMessage } from 'element-plus'
import type { FormInstance, FormRules } from 'element-plus'

const api = useApi()
const { data: feedbackTypesData } = await useAsyncData('feedback-types', () =>
  api.get<any[]>('/feedback/types'),
)
const { data: feedbackListData } = await useAsyncData('feedback-list', () =>
  api.get<any[]>('/feedback'),
)
const feedbackTypes = computed(() => feedbackTypesData.value ?? [])
const feedbackList = computed(() => feedbackListData.value ?? [])
