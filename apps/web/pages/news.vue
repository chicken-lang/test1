...
// 数据源: useApi → /api/news + /api/hot-keywords（SSR 预取）
const api = useApi()
const { data: newsData } = await useAsyncData('news', () =>
  api.get<{ list: any[]; total: number }>('/news'),
)
const { data: hotData } = await useAsyncData('news-hot-keywords', () =>
  api.get<string[]>('/hot-keywords'),
)
const newsList = computed(() => newsData.value?.list ?? [])
const hotKeywords = computed(() => hotData.value ?? [])
