...
// 数据源: useApi → /api/calendar（SSR 预取，返回 {schedule, calendar, bus, phones}）
const api = useApi()
const { data } = await useAsyncData('calendar', () => api.get<any>('/calendar'))
const classSchedule = computed(() => data.value?.schedule ?? [])
const schoolCalendar = computed(() => data.value?.calendar ?? [])
const busSchedule = computed(() => data.value?.bus ?? [])
const departmentPhones = computed(() => data.value?.phones ?? [])
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="(item, idx) in classSchedule" :key="idx" :class="{ highlight: item.note?.includes('课间') || item.note?.includes('午休') }">
                <td class="td-section">{{ item.section ?? item.name ?? '-' }}</td>
                <td class="td-time">{{ item.time ?? (item.startDate ? `${item.startDate} ~ ${item.endDate || ''}` : '-') }}</td>
                <td class="td-note">{{ item.note ?? (item.weeks ? `${item.weeks}周` : '-') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="panel-card">
          <ul class="calendar-list">
            <li v-for="(item, idx) in schoolCalendar" :key="idx" class="calendar-item">
              <div class="cal-week">{{ item.week ?? item.type ?? '-' }}</div>
              <div class="cal-info">
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="(bus, idx) in busSchedule" :key="idx">
                ...
                <td>{{ bus.direction ?? '-' }}</td>
                <td class="td-time">{{ bus.morning ?? bus.time ?? '-' }}</td>
