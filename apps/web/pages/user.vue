...
// 数据源: useApi → /api/user/profile|messages|favorites|history|feedback|subscriptions（SSR 预取）
...
// ========== 数据获取(SSR 并行预取) ==========
const { data: userProfileData } = await useAsyncData('user-profile', () =>
  api.get<any>('/user/profile'),
)
const { data: userMessagesData } = await useAsyncData('user-messages', () =>
  api.get<any[]>('/user/messages'),
)
const { data: userFavoritesData } = await useAsyncData('user-favorites', () =>
  api.get<any[]>('/user/favorites'),
)
const { data: userHistoryData } = await useAsyncData('user-history', () =>
  api.get<any[]>('/user/history'),
)
const { data: userFeedbackData } = await useAsyncData('user-feedback', () =>
  api.get<any[]>('/user/feedback'),
)
const { data: userSubscriptionsData } = await useAsyncData('user-subscriptions', () =>
  api.get<any[]>('/user/subscriptions'),
)

const userProfile = computed(() => userProfileData.value ?? {})
// 后端/BFF 返回 { list, total, page, pageSize, unreadCount } 对象,需取 list 字段;
// mock 路径下可能直接返回数组,故兼容两种形态
const userMessages = computed(() => {
  const data = userMessagesData.value as any
  if (!data) return []
  if (Array.isArray(data)) return data
  return data.list ?? []
})
const userFavorites = computed(() => {
  const data = userFavoritesData.value as any
  if (!data) return []
  if (Array.isArray(data)) return data
  return data.list ?? data ?? []
})
const userFeedback = computed(() => {
  const data = userFeedbackData.value as any
  if (!data) return []
  if (Array.isArray(data)) return data
  return data.list ?? data ?? []
})
...
// 浏览历史仅展示最近 5 条
const recentHistory = computed(() => (userHistoryData.value ?? []).slice(0, 5))

// 订阅列表(ref 包装,支持 el-switch 双向绑定;从 API 数据初始化可变副本)
const subscriptions = ref<any[]>([])
watchEffect(() => {
  const data = userSubscriptionsData.value
  if (data && data.length && subscriptions.value.length === 0) {
    subscriptions.value = data.map((s) => ({ ...s }))
  }
})
...
// 消息类型 -> el-tag 配置(system=蓝/notice=金/feedback=绿/approval-todo=蓝)
// 后端 Message.type 枚举包含 approval-todo,见 prisma schema
const msgTagMap: Record<string, { type: 'primary' | 'warning' | 'success' | 'info'; label: string }> = {
  system: { type: 'primary', label: '系统' },
  notice: { type: 'warning', label: '通知' },
  feedback: { type: 'success', label: '反馈' },
  'approval-todo': { type: 'info', label: '待办' },
}

// 安全获取消息标签(未知 type 兜底,避免 msgTagMap[xxx].type 抛错导致 SSR 500)
const getMsgTag = (type: string) =>
  msgTagMap[type] ?? { type: 'info' as const, label: type || '其他' }

// 消息是否未读(兼容后端 isRead 与 mock read 字段)
const isMsgUnread = (msg: any) => (msg.isRead ?? !msg.read) === false

// 消息日期(兼容后端 createdAt 与 mock date 字段)
const getMsgDate = (msg: any) => {
  const d = msg.createdAt ?? msg.date
  if (!d) return ''
  // ISO 字符串截取 YYYY-MM-DD
  return typeof d === 'string' && d.length >= 10 ? d.slice(0, 10) : String(d)
}
...
const onRemoveFavorite = async (id: number) => {
  try {
    ...
    const list = userFavoritesData.value ?? []
    userFavoritesData.value = list.filter((fav: any) => fav.id !== id)
  } catch (e) {
...
// 订阅切换
const onSubscriptionChange = async (sub: any, active: boolean) => {
  try {
    if (active) {
      await api.post('/user/subscriptions', {
        targetType: sub.type,
        targetId: String(sub.id || sub.name),
        targetName: sub.name,
      })
      ElMessage.success(`已订阅:${sub.name}`)
    } else {
      await api.delete(`/user/subscriptions/${sub.id}`)
      ElMessage.success(`已取消订阅:${sub.name}`)
    }
  } catch (e) {
    ElMessage.error('订阅操作失败,请重试')
  }
}
              ...
              <div class="tab-section">
                <ul class="msg-list">
                  <li
                    ...
                    :key="msg.id"
                    class="msg-item"
                    :class="{ 'is-unread': isMsgUnread(msg) }"
                  >
                    <div class="msg-item__head" @click="toggleMsg(msg.id)">
                      <span class="msg-item__dot" />
                      <span class="msg-item__title">{{ msg.title }}</span>
                      <el-tag size="small" :type="getMsgTag(msg.type).type" effect="light">
                        {{ getMsgTag(msg.type).label }}
                      </el-tag>
                      <span class="msg-item__date">{{ getMsgDate(msg) }}</span>
                      <Icon
              ...
              <div class="tab-section">
                ...
                <ul class="sub-list">
                  <li v-for="(sub, idx) in subscriptions" :key="idx" class="sub-item">
                    ...
                    <el-switch
                      v-model="sub.active"
                      @change="(val: string | number | boolean) => onSubscriptionChange(sub, Boolean(val))"
                    />
                  </li>
                </ul>
              </div>
