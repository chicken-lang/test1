...
// 数据源: useApi → /api/disclosure/report（SSR 预取）
const api = useApi()
const { data } = await useAsyncData('disclosure-report', () => api.get<any>('/disclosure/report'))
const disclosureReport = computed(() => data.value ?? null)
...
.report-summary-card {
  ...
  &::after {
    ...
    height: 200px;
    border-radius: 50%;
    background: rgba(0, 115, 189, 0.08); // VI 主色半透明装饰圆(无渐变)
    pointer-events: none;
  }
}
